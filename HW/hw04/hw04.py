...
def balanced(m):
    ...
    True
    """
    return ( length(left(m))*total_mass(end(left(m))) == length(right(m))*total_mass(end(right(m))) ) and (balanced(end(left(m))) if is_mobile(end(left(m))) else True)  and (balanced(end(right(m))) if is_mobile(end(right(m))) else True)
...
def berry_finder(t):
    ...
    True
    """
    return label(t) == 'berry' or (True if True in [berry_finder(x) for x in branches(t)] else False)
...
def max_path_sum(t):
    ...
    17
    """
    return label(t) + (max([max_path_sum(x) for x in branches(t)]) if not is_leaf(t) else 0)
