def divide(quotients, divisors):
    ...
    """
    return {quotients[i]:[divisors[j] for j in range(len(divisors)) if not divisors[j]%quotients[i] ]for i in range(len(quotients))}
...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            for k in range(1,amount // price + 1): #坑-->如果为最后水果选择时,range(1,1)是不会执行的,即无法得到最终结果!!!
