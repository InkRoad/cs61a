...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    fliter_paragraphs = [x for x in paragraphs if select(x)]
    if k > len(fliter_paragraphs) - 1:
        return ''
    return fliter_paragraphs[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    that paragraph contains one of the words in SUBJECT.
        如果段落中含有subject的词，就返回True否则返回False
    Arguments:
    ...
    # BEGIN PROBLEM 2
    def fliter(paragraph):
        words = lower(remove_punctuation(paragraph)).split() #这里是将一个语句的单词拆成一个列表
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    actual_count = hitted_count = 0.0
    typed_len,source_len = len(typed_words),len(source_words)

    if not source_len + typed_len :
        return 100.0
    elif source_len * typed_len == 0 :
        return 0.0
    ...
    actual_count += typed_len-source_len if typed_len > source_len else 0
    return hitted_count/actual_count * 100.0
    # END PROBLEM 3

def wpm(typed, elapsed):
    """Return the words-per-minute (WPM) of the TYPED string.
        elapsed--多少秒
        speed = len(typed)/5.0 * 60.0/elapsed
    Arguments:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    difference is greater than LIMIT, return TYPED_WORD instead.
        ...
        wordlist中含有typedword直接返回
        word_list中和typed_word的diff最小的，多个最小取最前面
        若最小差异大于limit就直接返回typed_word
    Arguments:
    ...
    min_index,min_value = 10000,99999
    for i in range(len(word_list)):
        diff = diff_function(typed_word,word_list[i],limit)
        if diff <= limit and diff < min_value :
                min_index,min_value = i,diff
    return typed_word if min_index == 10000 else word_list[min_index]
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    hitted,total = 0.0,float(len(source))
    len_typed = len(typed)
    ...
    upload({'id': user_id,'progress':hitted/total})
    return hitted/total
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamps in timestamps_per_player:
        len_stamps = len(timestamps)
        temp = []
        for i in range(1,len_stamps):
            temp.append(timestamps[i]-timestamps[i-1])
        times.append(temp)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    players = [[] for i in player_indices]
    for i in word_indices:
        min_index = 0
        for index in player_indices:
            if times[index][i] < times[min_index][i]:
                min_index = index
        players[min_index].append(words[i])
    return players 
    # END PROBLEM 10
