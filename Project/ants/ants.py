...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        只创建一个Place，exit和entrance都为None
        exit是左侧的Place entrance是右侧的格子 每次init创建的是右侧的格子
        """
        self.name = name
        self.exit = exit
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container ^ self.is_container:
                container,botainer = place.ant if place.ant.is_container else (self if self.is_container else None),place.ant if not place.ant.is_container else (self if not self.is_container else None)
                if container.can_contain(botainer):
                    container.store_ant(botainer)
                    place.ant = container
                    flag = 0
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        # BEGIN Problem 3 and 4--take long time 未完全理解题意
        current = self.place
        counter = 0
        while current and not current.is_hive:
            if random_bee(current.bees):
                if counter >= self.lower_bound and counter <= self.upper_bound:
                    return random_bee(current.bees)
            current,counter = current.entrance,counter+1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5--读题哥哥，浪费时间长！！！
        place = self.place
        Ant.reduce_health(self,amount)
        actual = amount + (self.damage if self.health <= 0 else 0)
        
        for x in list(place.bees):
            Insect.reduce_health(x,actual)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown == 0 and (bee := random_bee(self.place.bees)):
        ...
        while current:
            if current.ant:
                if current.ant.is_container and current.ant.ant_contained:
                    if current.ant.ant_contained.can_double:
                        Ant.double(current.ant.ant_contained)
                        current.ant.ant_contained.can_double = 0
                if current.ant.can_double:
                    Ant.double(current.ant)
                    current.ant.can_double = 0
