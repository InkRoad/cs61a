    ...
    if scheme_symbolp(signature):
        ...
        while p is not nil:
            if not scheme_symbolp(p.first):
                raise SchemeError('Not valid formals')
            p = p.rest
            
        res = LambdaProcedure(signature.rest,expressions.rest,env)   #注意expressions.rest是整个表达式 其元素是每个表达式;expressions.rest.first则是进入表达式内了，其元素是oprators或oprand
        env.define(signature.first,res)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    res = LambdaProcedure(expressions.first,expressions.rest,env)
    return res
    # END PROBLEM 7
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            test = scheme_eval(clause.first, env) if clause.first != 'else' else True
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            return eval_all(clause.rest,env) if clause.rest is not nil else test  #上面else语句修改一下即可 
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    while bindings is not nil:  #bindings代表的是所有赋值表达式构成的链表，.first访问的就是其中的一个表达式.first.first是元素
        validate_form(bindings.first,2,2)
        names,vals = Pair(bindings.first.first,names),Pair(scheme_eval(bindings.first.rest.first,env),vals)  #注意评估原子单位
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    res = MuProcedure(expressions.first,expressions.rest)
    return res
    # END PROBLEM 11
