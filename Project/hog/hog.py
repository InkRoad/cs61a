...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    player_digit,opponent_digit = player_score%10,(opponent_score//10)%10
    return 3*(abs(player_digit-opponent_digit)) if 3*(abs(player_digit-opponent_digit)) > 1 else 1
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    i,max_res,max_i = 1,0,int()
    while i <= 10:
        res = make_averaged(roll_dice,times_called)(i,dice)
        if res > max_res :
            max_res,max_i = res,i
        i += 1
    return max_i
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    last_score = score
    score = sus_update(0,last_score,opponent_score)
